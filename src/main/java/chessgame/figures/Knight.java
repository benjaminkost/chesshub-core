...
public class Knight extends ChessFigure {
	 ...
	 */
	private final static String BLACK_KNIGHT_PATH = "chessgame/figures/images/BlackKnight.png";

	/**
	 * The image path for the white figure
	 */
	private final static String WHITE_KNIGHT_PATH = "chessgame/figures/images/WhiteKnight.png";
	...
	public String getPath() {
		if (side == ChessColor.BLACK)
			return BLACK_KNIGHT_PATH;
		else
			return WHITE_KNIGHT_PATH;
	}
	...
	public List<ChessTile> getStepOptions(ChessBoard board) {
		...
		if (row >= 2) {
			if (column >= 1)
				if (board.getTile(row - 2, column - 1).getFigure() == null
						|| board.getTile(row - 2, column - 1).getFigure().getSide() != side)
		...
		if (column >= 2) {
			if (row >= 1)
				if (board.getTile(row - 1, column - 2).getFigure() == null
						|| board.getTile(row - 1, column - 2).getFigure().getSide() != side)
