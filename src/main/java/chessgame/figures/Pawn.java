...
public class Pawn extends ChessFigure {
	 ...
	 */
	private final static String BLACK_PAWN_PATH = "chessgame/figures/images/BlackPawn.png";

	/**
	 * The image path for the white figure
	 */
	private final static String WHITE_PAWN_PATH = "chessgame/figures/images/WhitePawn.png";
	...
	public List<ChessTile> getStepOptions(ChessBoard board) {
		...
		if (row != 0) {
			...
			if (column != 0 && board.getTile(row - 1, column - 1).getFigure() != null
					&& board.getTile(row - 1, column - 1).getFigure().getSide() != side)
				steps.add(board.getTile(row - 1, column - 1));
			if (column != board.getM() - 1 && board.getTile(row - 1, column + 1).getFigure() != null
					&& board.getTile(row - 1, column + 1).getFigure().getSide() != side)
				steps.add(board.getTile(row - 1, column + 1));
			if (row == board.getN() - 2 && board.getTile(row - 1, column).getFigure() == null
					&& board.getTile(row - 2, column).getFigure() == null)
