...
import chessgame.figures.Bishop;
import chessgame.figures.ChessFigure.ChessColor;
import chessgame.figures.King;
import chessgame.figures.Knight;
import chessgame.figures.Pawn;
import chessgame.figures.Queen;
import chessgame.figures.Rook;
import chessgame.util.Message;
import chessgame.util.Message.Type;
import chessgame.util.NetworkListener;
import chessgame.util.NetworkUtils;
	...
	class TileListener implements MouseListener {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if (stepOptions.isEmpty()) {
				...
				if (!playersTurn)
					return;
				if (tile.getFigure().getSide() != playerSide)
					return;
				lastPressed = tile;
	...
	class MessageListener implements NetworkListener {
		...
		public void messageReceived(Message message) {
			...
			if (message.getType() == Type.STEP) {
				...
				if (parts.length < 4)
					return;
				ChessTile from = getTile(getN() - 1 - Integer.parseInt(parts[0]),
						getM() - 1 - Integer.parseInt(parts[1]));
				ChessTile to = getTile(getN() - 1 - Integer.parseInt(parts[2]),
						getM() - 1 - Integer.parseInt(parts[3]));
				to.setFigure(from.getFigure());
				from.setFigure(null);
				nextPlayer();
			} else if (message.getType() == Type.SIDE) {
				if (message.getString().equals("WHITE"))
					playerSide = ChessColor.WHITE;
				else
					playerSide = ChessColor.BLACK;
				playersTurn = playerSide == ChessColor.BLACK ? false : true;
